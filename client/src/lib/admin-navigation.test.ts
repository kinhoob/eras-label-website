...
describe("admin navigation groups", () => {
  ...
  it("não exibe Gestão de Equipa para subadministradores", () => {
    const groups = buildAdminNavGroups(false);
    const labels = groups.flatMap((group) => group.items.map((item) => item.label));

    expect(labels).not.toContain("Gestão de Equipa");
  ...
  it("encontra a categoria da página ativa", () => {
    ...
    expect(getAdminNavGroupId(groups, "Gestão de Equipa")).toBe("settings");
